...
// B-1. Using only ONE line in the variablesExercise.js file, create the following const variables (quarter = 0.25, dime = 0.10, nickel = 0.05, & penny = 0.01). Also, only allowed to use the "const" keyword ONE time.
const quarter = 0.25, dime = 0.10, nickel = 0.05, penny = 0.01;
//or
//const [quarter, dime, nickel, penny] = [0.25, 0.10, 0.05, 0.01];

//B-2. On the next line create a console log that displays all four variable values (0.25, 0.10, etc.) on ONE line in terminal when the file is run.
console.log(quarter, dime, nickel, penny);

// B-3. Create a new variable with the type of "let" and a name of "letters". Set the value of the variable to the following string "qwertyuiop".
let letters = "qwertyuiop";

// B-4. Using index and a console log, have the "p" at the end of the "letters" string variable display in the terminal when you run the file. Also, you must use the length property when selecting the "p" with index.
// NOTE: Get the position of p by running letters.length
